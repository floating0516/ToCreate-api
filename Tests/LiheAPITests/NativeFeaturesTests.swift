...
final class NativeFeaturesTests: XCTestCase {
    ...
    func testWidgetSourceFilesDefineWidgetKitExtension() throws {
        ...
        XCTAssertTrue(widget.contains("galleryPreviewSnapshot"))
        XCTAssertTrue(widget.contains("TimelineProvider"))
        XCTAssertFalse(widget.contains("(try? store.load()) ??"))
        XCTAssertTrue(widget.contains("snapshot: try? store.load()"))
        XCTAssertEqual(widget.components(separatedBy: "snapshot: try? store.load()").count - 1, 2)
        XCTAssertFalse(widget.contains("balance: 399_478.22"))
    ...
    func testReleaseScriptAutomatesSafeGitHubReleaseFlow() throws {
        ...
        XCTAssertTrue(script.contains("Usage: ./scripts/release.sh [--dry-run] <version> <release-notes>"))
        XCTAssertTrue(script.contains("DRY_RUN=0"))
        XCTAssertTrue(script.contains("if [[ \"${1:-}\" == \"--dry-run\" ]]"))
        XCTAssertTrue(script.contains("Working tree has uncommitted changes; dry run will not modify them."))
        ...
        XCTAssertTrue(script.contains("PROJECT_FILE=\"$ROOT/ToCreate.xcodeproj/project.pbxproj\""))
        XCTAssertTrue(script.contains("MARKETING_VERSION = $VERSION;"))
        XCTAssertTrue(script.contains("CURRENT_PROJECT_VERSION = $NEXT_BUILD;"))
        XCTAssertTrue(script.contains("Release summary"))
        XCTAssertTrue(script.contains("Current version: $CURRENT_VERSION ($CURRENT_BUILD)"))
        XCTAssertTrue(script.contains("New version:     $VERSION ($NEXT_BUILD)"))
        XCTAssertTrue(script.contains("Release notes:   $RELEASE_NOTES"))
        XCTAssertTrue(script.contains("Dry run only. No files were changed."))
    ...
    func testPackageScriptBuildsXcodeAppWithoutWidgetBundleWhileFeatureIsPaused() throws {
        ...
        XCTAssertTrue(script.contains("-destination \"generic/platform=macOS\""))
        XCTAssertTrue(script.contains("ONLY_ACTIVE_ARCH=NO"))
        XCTAssertTrue(script.contains("ARCHS=\"arm64 x86_64\""))
